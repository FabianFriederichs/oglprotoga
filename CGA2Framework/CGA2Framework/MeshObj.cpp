...
void MeshObj::setData(const MeshData &meshData) {
  ...
  std::copy(meshData.vertex_position.begin(), meshData.vertex_position.end(), vertex_position.begin());
  std::vector<GLfloat> vertex_normal;
  if (vertexNormalSize > 0) {
	  
	std::copy(meshData.vertex_normal.begin(), meshData.vertex_normal.end(), vertex_normal.begin());
  }
  std::vector<GLfloat> vertex_texcoord;
  if (vertexTexcoordSize > 0) {
    
	std::copy(meshData.vertex_texcoord.begin(), meshData.vertex_texcoord.end(), vertex_texcoord.begin());
  }
  std::vector<GLuint> indices;
  std::copy(meshData.indices.begin(), meshData.indices.end(), indices.begin());
